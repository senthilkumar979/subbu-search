...
export interface IAC174 extends Document {
  acNo: number;
  partNo: number;
  slNoInPart: number;
  houseNo?: string;
...
const AC174Schema: Schema = new Schema(
  {
...
AC174Schema.index({ acNo: 1, partNo: 1 });
AC174Schema.index({ houseNo: 1, sectionNo: 1 });
AC174Schema.index({ fmNameV2: 'text', rlnFmNmV2: 'text', fmNameEn: 'text', rlnFmNmEn: 'text' });

export default mongoose.models.AC174 || mongoose.model<IAC174>('AC174', AC174Schema);
